...
namespace Case.FrontEnd
{
    public class MemberManager : UserManager<Member>
    {
        ...
        {
            var store = (IUserStore<Member>) DependencyResolver.Current.GetService(typeof(IUserStore<Member>));
        ...
        public override async Task<IdentityResult> CreateAsync(Member user, string password)
        {
            ...
            var exists = (await Store.FindByNameAsync(user.Ssn) != null); // Name == ssn
            if (exists)
            {
                return IdentityResult.Failed("Member already exists");
            }
